...
<body class="frontpage">
    ...
    <div class="container breather" x-data="{step: 'applicationData'}">
        <div class="row">
            <div class="col-md-4">
                ...
                <ul class="component-application-steps">
                    <li x-bind:class="step === 'applicationData' ? 'component-application-steps__step is-active' : 'component-application-steps__step'" x-on:click="step = 'applicationData'">Rad</li>
                    <li x-bind:class="step === 'contestantData' ? 'component-application-steps__step is-active' : 'component-application-steps__step'" x-on:click="step = 'contestantData'">Natjecatelji</li>
                    <li x-bind:class="step === 'mentorData' ? 'component-application-steps__step is-active' : 'component-application-steps__step'" x-on:click="step = 'mentorData'">Mentori</li>
                </ul>
            ...
            <div class="col-md-8">
                <form action="/editApplication" method="post">
                    ...
                    <div class="applicationData" data-aos="fade-up" x-show="step === 'applicationData'">
                        ...
                        <select name="category" id="category" class="form-control">
                            ...
                            ?>
                            <option disabled <?php if (!$Data->category) {
                                                    echo 'selected';
                                                } ?>>Odaberite...</option>
                            <option value="humanisticke" <?php if ($Data->category == 'humanisticke') {
                                                                echo 'selected';
                                                            } ?>>Humanističke znanosti</option>
                            <option value="kemija" <?php if ($Data->category == 'kemija') {
                                                        echo 'selected';
                                                    } ?>>Kemija i srodne znanosti</option>
                            <option value="fizika" <?php if ($Data->category == 'fizika') {
                                                        echo 'selected';
                                                    } ?>>Fizika i srodne znanosti</option>

                            <option value="biomed" <?php if ($Data->category == 'biomed') {
                                                        echo 'selected';
                                                    } ?>>Biologija, medicina i srodne znanosti</option>
                            <option value="mathcs" <?php if ($Data->category == 'mathcs') {
                                                        echo 'selected';
                                                    } ?>>Matematika, informatika, elektrotehnika i srodne znanosti</option>
                            <option value="ostale" <?php if ($Data->category == 'ostale') {
                                                        echo 'selected';
                                                    } ?>>Ostale znanosti / Interdisciplinaran rad</option>
                            <option value="originalresearch" <?php if ($Data->category == 'originalresearch') {
                                                                    echo 'selected';
                                                                } ?>>Originalan istraživački rad</option>
                        </select>
                        ...
                        <?php if ($Application->vmssID && ($route[2] ?? '') != 'video') {
                            ...
                            echo '</div>';
                        } elseif ($vmssUnavailable ?? false) {
                            echo '<div>';
                            echo '<p>Sustav za videozapise nije dostupan. Morat ćeš dodati video kasnije</p>';
                        } else echo '<div id="drag-drop-area"></div>';
                        ?>
                        <p>Video zapis mora biti ispod 500 MB. Dozvoljeni formati: .webm, .mkv, .avi, .wmv, .mov, .mp4, .m4v, .mpg, .mp2, .mpeg, .mpv, .m2v, .m4v</p>
                        <hr>
                        ...
                        ?>
                        <a class="btn btn-primary" role="button" x-on:click="step = 'contestantData'">Nastavi na natjecatelje &rarr;</a>
            ...
            <div class="contestantData" style="display:none;" data-aos="fade-up" x-show="step === 'contestantData'">
                ...
                <?php if ($User->aai == json_decode($Application->teamMembers)->secondary->aai ?? null) {
                    $disabled = '';
                    $Competitor = $User;
                } else {
                    $disabled = 'readonly onclick="return false;"';
                    $Competitor = new User($pdo);
                    $Competitor->aai = json_decode($Application->teamMembers)->secondary->aai ?? null;
                    $Competitor->load();
                }
                ?>
                ...
                <hr>
                <a class="btn btn-primary" role="button" x-on:click="step = 'mentorData'">Nastavi na mentore &rarr;</a>

                <div class="breather">
                    <div class="flex flex--align-center icon"><?php echo Component::render('icon', 'fingerprint'); ?> <b>Tvoja privatnost</b></div>
                    <p>Objavit ćemo samo tvoje ime, prezime i školu.</p>
                </div>
            </div>
            <div class="mentorData" style="display:none;" data-aos="fade-up" x-show="step === 'mentorData'">
                ...
                <br><br>
                <b>Mentor 2</b><br>
                <p>Preskoči ovaj dio ako imaš jednog mentora.</p>
                ...
                <div class="breather">
                    <div class="flex flex--align-center icon"><?php echo Component::render('icon', 'fingerprint'); ?> <b>Tvoja privatnost</b></div>
                    <p>Objavit ćemo samo ime i prezime mentora.</p>
                </div>
            </div>
        </div>
